        ...
        if (lastItem) {
          ...
          if (lastItem.outcome === GameResult.Tie) {
            if (lastItemInResults) {
              ...
              // if there is no last item in results, but there have been ties, create a
              // dummy item for the first cell and increement the logicalColumn
              logicalColumnNumber++;
              returnList.push({
    ...
    Object.values(columnDefinitions).forEach((bigRoadColumn) => {
      ...
      for (let m = 0; m < bigRoadColumn.logicalColumnDepth; m++) {
        ...
        if (rowMDepth >= 2) {
          ...
          if (
            kDistanceColumnDefinition != null &&
            leftColumnDefinition != null
          ) {
            if (
              kDistanceColumnDefinition.logicalColumnDepth ==
              leftColumnDefinition.logicalColumnDepth
            )
              outcome = 'red';
            else outcome = 'blue';
